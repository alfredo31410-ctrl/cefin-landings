export const PLATAFORMAS_WHATSAPP_INTENT_KEY =
  "cefin_plataformas_whatsapp_intent";
...
export function createPlataformasWhatsAppIntent() {
  try {
    ...
    window.sessionStorage.setItem(
      PLATAFORMAS_WHATSAPP_INTENT_KEY,
...
export function consumePlataformasWhatsAppIntent() {
  let serialized: string | null = null;
  try {
    serialized = window.sessionStorage.getItem(
      PLATAFORMAS_WHATSAPP_INTENT_KEY,
    );
    window.sessionStorage.removeItem(PLATAFORMAS_WHATSAPP_INTENT_KEY);
  } catch {
    return null;
  }
...
export function waitForPlataformasMetaPixel(
  onReady: () => void,
  onTimeout: () => void,
) {
  let attempts = 15;
