...
import {
  ...
  signDespiertaRegistrationProof,
...
async function handleCompletion(request: NextRequest) {
  ...
  const proof = verifyDespiertaRegistrationProof(
    request.cookies.get(DESPIERTA_REGISTRATION_COOKIE)?.value,
  );
  ...
  response.cookies.set(
    DESPIERTA_REGISTRATION_COOKIE,
    signDespiertaRegistrationProof({ ...proof, state: "sent" }),
    {
      ...despiertaRegistrationCookieOptions,
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("[despierta-registration] Completion failure:", message);
